...
import br.com.android.postovalle.extensions.toBase64
import br.com.android.postovalle.model.UrlServer
import br.com.android.postovalle.model.User
import br.com.android.postovalle.parse.Json
import br.com.android.postovalle.services.HttpClient
import br.com.android.postovalle.services.HttpClientWeb
import br.com.android.postovalle.utils.*
...
/**
 * POSTO VALLE APP VERSION 4
 * HIBRYD SINGLE ACTIVITY + WEBVIEW LOADING PAGES
 * Gradle gradle-6.5
 * Gradle android 4.1.1
 * Kotlin 1.4
 * URL DEFAULT: https://adm.bunkerapp.com.br/app/intro.do?key=Ys%C2%A3p%C2%A3u6k50jp%C2%A3M%C2%A2
            ...
            HttpClient.getInstance.postAsync(Request.Method.POST, mUrlUserSearchKeyData, postparams, object : Callback, okhttp3.Callback {
                ...
                override fun onResponse(call: Call, response: okhttp3.Response) {
                    if (response.isSuccessful && response.code == 200) {
                        ...
                        completion(false, User())
                        Log.d(getString(R.string.Error_With_PostoValle),"Aconteceu algum problema na conexão...")
                    }
                }
            })
        }
        ...
        private fun sendOneSignalDataToRedeDuque(userLogged : User, completion: (success: Boolean) -> Unit) {
            ...
            HttpClient.getInstance.postAsync(Request.Method.POST, mUrlUserPushDataInformation, postparams, object : Callback, okhttp3.Callback {
                ...
                override fun onResponse(call: Call, response: okhttp3.Response) {
                    if (response.isSuccessful && response.code == 200) {
                        //get data user from idU Key
                        Log.d(getString(R.string.Success_To_PostoValle),"Enviados dados OneSignal com sucesso...")
                        completion(true)

                    } else {
                        completion(false)
                        Log.d(getString(R.string.Data_Sent_to_PostoValle),"Aconteceu algum problema na conexão...")
                    }
                }
            })
        }
                    ...
                    processRedeDuqueUrlKey(IDUkey!!, completion = { success: Boolean, user: User? ->
                        if (success){
                            ...
                            sendOneSignalDataToRedeDuque(userLogged!!, completion = {
                                if (it) Log.d(getString(R.string.Data_Sent_to_PostoValle), "Dados OneSignal Enviados para Rede Duque!")
                            })
                        }
                    })
                }
